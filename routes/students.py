    ...
    if 'student_username' and 'student_name' in session:
        ...
        if request.method=="POST":
            ...
            if type=="skip":
                ...
                return redirect(url_for('questions',subject_id=student.classes.subjects[recent_subject.subjects_counts].id
                                                    ,prof_id=int(prof_id)
                                                    ,classe_id=classe.id))
        else:
...
def reset_password(id):
    if 'school_username' and 'school_name' in session:
        ...
        if student in students:
            ...
            return F"envoyé avec succées A {student.user.email}"
        else:
            return render_template("error_page.html")
    else:
        return redirect(url_for("home"))